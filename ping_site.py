...
def get_site_emails(site):
    rows = c.execute('SELECT e.email_address from email e JOIN email_site es ON es.email_id = e.id JOIN sites s on es.site_id = s.id WHERE s.url=?', (site,))
    conn.commit()
